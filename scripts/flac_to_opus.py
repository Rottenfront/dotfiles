...
def convert_file(args: Tuple[str, str, bool]) -> Tuple[str, bool, str]:
    """
    Convert a single FLAC file to OPUS.
...
def process_album_dir(args: Tuple[str, str, bool]) -> Tuple[int, int, int, int]:
    ...
    flac_files = list(source_path.glob('*.flac'))
    total = len(flac_files)

    # Prepare conversion tasks
    tasks = []
    for flac_file in flac_files:
        rel_path = flac_file.relative_to(source_path.parent)
        output_file = dest_path.parent / rel_path.with_suffix('.opus')
        tasks.append((str(flac_file), str(output_file), mirror_mode))
...
def main():
    ...
    album_dirs = set()
    for flac_file in source_dir.rglob('*.flac'):
